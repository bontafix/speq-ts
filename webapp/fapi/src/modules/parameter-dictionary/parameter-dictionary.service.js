...
exports.ParameterDictionaryService = void 0;
...
class ParameterDictionaryService {
    ...
    async getByKey(key) {
      ...
      `, [key]);
        if (result.rows.length === 0) {
            throw new app_error_1.NotFoundError("Parameter dictionary entry not found");
        }
    ...
    async create(data) {
        if (!data.key || data.key.trim().length === 0) {
            throw new app_error_1.ValidationError("Key is required");
        }
        if (!data.labelRu || data.labelRu.trim().length === 0) {
            throw new app_error_1.ValidationError("LabelRu is required");
        }
        if (!data.category || data.category.trim().length === 0) {
            throw new app_error_1.ValidationError("Category is required");
        }
        if (!data.paramType || data.paramType.trim().length === 0) {
            throw new app_error_1.ValidationError("ParamType is required");
        }
        if (!data.sqlExpression || data.sqlExpression.trim().length === 0) {
            throw new app_error_1.ValidationError("SqlExpression is required");
        }
        // Проверка существования
        const existing = await this.fastify.db.query(`SELECT key FROM parameter_dictionary WHERE key = $1`, [data.key]);
        if (existing.rows.length > 0) {
            throw new app_error_1.ValidationError("Parameter dictionary entry with this key already exists");
        }
        const result = await this.fastify.db.query(`
    ...
    async update(key, data) {
        ...
        if (data.labelRu !== undefined) {
            if (data.labelRu.trim().length === 0) {
                throw new app_error_1.ValidationError("LabelRu cannot be empty");
            }
        ...
        if (data.category !== undefined) {
            if (data.category.trim().length === 0) {
                throw new app_error_1.ValidationError("Category cannot be empty");
            }
        ...
        if (data.paramType !== undefined) {
            if (data.paramType.trim().length === 0) {
                throw new app_error_1.ValidationError("ParamType cannot be empty");
            }
        ...
        if (data.sqlExpression !== undefined) {
            if (data.sqlExpression.trim().length === 0) {
                throw new app_error_1.ValidationError("SqlExpression cannot be empty");
            }
...
exports.ParameterDictionaryService = ParameterDictionaryService;
//# sourceMappingURL=parameter-dictionary.service.js.map